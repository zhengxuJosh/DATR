...
#-------------------Cityscapes dataset path-------------------#
root_city = '/hy-tmp/workplace/xuzheng/data/cps'
#-------------------Cityscapes dataset path-------------------#
...
class CityDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, split='train_aug',use_difficult=False, is_train = True, augment = None, cutmix=False, base_size=520, crop_size=480, file_length=None, norm_mean=[0.5, 0.5, 0.5], norm_std=[0.5, 0.5, 0.5]
                , pass_like=False):
        self.root_city= root_city
        self.txt_path = root_dir
        self.use_difficult=use_difficult
        self.imgset=split
        self.cutmix = cutmix
        self.num_class = 19
        self._imgpath = os.path.join(self.root_city, "", "%s")
        self._labelpath = os.path.join(self.root_city, "", "%s")

        self._imgsetpath = os.path.join(self.txt_path, "%s.txt")
        self._labelsetpath = os.path.join(self.txt_path, "%s.txt")

        self.pass_like = pass_like
        ...
        self.name2id=dict(zip(CityDataset.CLASSES_NAME,range(len(CityDataset.CLASSES_NAME))))
    ...
    def __getitem__(self,index):
        ...
        if self.train:
            if self.pass_like:
                _img, _target = self._passlike_transform(_img, _target) #
            else:    
                _img, _target = self._sync_transform( _img, _target) # self._passlike_transform(_img, _target) #
        else:
    ...
    def _passlike_transform(self, img, mask):
        crop_size = 400
        h, w = img.size #[2048,1024]
        x1 = random.randint(0, w - crop_size)
        img = img.crop((0, x1, h, x1+crop_size))
        mask = mask.crop((0, x1, h, x1+crop_size))
