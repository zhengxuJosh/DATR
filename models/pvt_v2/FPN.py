...
class FPN(nn.Module):
    ...
    def __init__(self, num_classes, back_bone='resnet', pretrained=True):
        ...
        self.back_bone = pvt_v2_b0()
        ckpt = torch.load('/hpc/users/CONNECT/xuzheng/C2VKD/models/ptmodel/pvt_v2_b0.pth',map_location='cpu')
        self.back_bone.load_state_dict(ckpt, strict=False)
        ...
        # Lateral layers
        self.latlayer1 = nn.Conv2d(160, 32, kernel_size=1, stride=1, padding=0)
		...
		# Semantic branch
        self.semantic_branch = nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1)
        self.conv3 = nn.Conv2d(16, self.num_classes, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        # [2, 256, 16, 16] -> [2, 256, 32, 32]
        feat = F.interpolate(low_level_features[3], size=(32,32), mode='bilinear', align_corners=True)
        return self._upsample(self.conv3(s2 + s3 + s4 + s5), 4 * h, 4 * w), feat
